...
nc$safemode = FALSE
	...
	#----------------------------------------------------------------------------
	if( nc$safemode ) {
		rv = .C("R_nc4_close", as.integer(nc$id), PACKAGE="ncdf4header")
...
ncvar_get <- function( nc, varid=NA, start=NA, count=NA, verbose=FALSE, signedbyte=TRUE, collapse_degen=TRUE ) {
	...
	#----------------------------------------------------------------
	if( nc$safemode ) {
		trv = .C("R_nc4_close", as.integer(nc$id), PACKAGE="ncdf4header")
		nc$id = -1	# invalidate this ID since it's not valid any more (duh)
		}
...
nc_sync <- function( nc ) {
	...
	rv = .C("R_nc4_sync", as.integer(ncid2use), PACKAGE="ncdf4header")
}
...
nc_redef <- function( nc ) {
	...
	rv = .C("R_nc4_redef", as.integer(ncid2use), PACKAGE="ncdf4header")
}
...
nc_enddef <- function( nc ) {
	...
	rv = .C("R_nc4_enddef", as.integer(ncid2use), PACKAGE="ncdf4header")
...
nc_close <- function( nc ) {
	...
	rv = .C("R_nc4_close", as.integer(ncid2use), PACKAGE="ncdf4header")
